...
# MT依赖包
from magenta.models.rl_tuner.MT.model_transformer import MusicTransformerDecoder
...
class RLTuner(object):
    ...
    def __init__(self, output_dir,
        ...
        with self.graph.as_default():
            ...
            self.output_every_nth = output_every_nth  # 每n步，输出模型的评估
    ...
    def initialize_internal_models_graph_session(self,
        ...
        """
        初始化RNN模型。通过在单独的函数中初始化，使模型可以继承该类，从而定义不同的q网络
        ...
        with self.graph.as_default():
            ...
            self.q_network = MusicTransformerDecoder(loader_path="model")
            ...
            #     hparams=self.note_rnn_hparams,
            #     note_rnn_type=self.note_rnn_type)
            self.target_q_network = MusicTransformerDecoder(loader_path="model")
            ...
            #     hparams=self.note_rnn_hparams,
            #     note_rnn_type=self.note_rnn_type)
            self.reward_rnn = MusicTransformerDecoder(loader_path="model")
    ...
    def train(self, num_steps=10000, exploration_period=5000, enable_random=True):
        ...
        Args:
          ...
          exploration_period: The number of steps over which the probability of
            ...
            在这么多step内，探索概率从1.0降低到random_action_probability
    ...
    def reward_music_theory(self, action):
        ...
        # 如果产生奖励，就打印出当前的奖励值，这样可以查看奖励值的更新过程
        reward += self.reward_tonic(action)
